...
public final class RSSAggregator {
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        if (rss.isTag() && rss.label().equals("rss")
            ...
            //Telling user the URL provided in file is valid
            out.println();
            out.println("The URL: " + url
                    + " contains a valid RSS 2.0 XML Document and " + "a valid "
                    + file + " file has been generated.");
            ...
            //Telling user the URL provided in file is not valid
            out.println();
            out.println("The URL: " + url
                    + " DOES NOT contain a valid RSS 2.0 XML Document and "
                    + "a valid " + file + " file HAS NOT been generated.");
        }
    ...
    public static void main(String[] args) {
        ...
        //Prompting user to input file and getting input
        out.print(
                "Enter the name of the XML file (including .xml extension): ");
        String xmlFile = in.nextLine();

        out.println();

        //Prompting user to enter output file and getting input
        out.print(
        ...
        //Declaring new variable to write to file
        SimpleWriter fileOut = new SimpleWriter1L(outputFile);

        //Declaring XMLTree type
        XMLTree feeds = new XMLTree1(xmlFile);

        //Checking if the root node is a tag and it equals feeds
        if (feeds.isTag() && feeds.label().equals("feeds")) {

            //Telling user that the XML file is valid
            out.println("The XML file is valid.");

            //Extracting information from title attribute
            String title = feeds.attributeValue("title");

            //Outputting to file in HTML format
            ...
            //Entering while loop to extract children of feeds
            int numberOfChild = feeds.numberOfChildren();
            int index = 0;
            while (index < numberOfChild) {

                //Declaring XMLTree type
                XMLTree feed = feeds.child(index);

                //Checking if the child is tag and it equals feed
                if (feed.isTag() && feed.label().equals("feed")) {

                    //Extracting attributes
                    String newsURL = feed.attributeValue("url");
                    String fileName = feed.attributeValue("file");
                    String newsName = feed.attributeValue("name");

                    //Calling processFeed method
                    processFeed(newsURL, fileName, out);

                    //Outputting to file in HTML format
                    fileOut.println("      <li><a href=\"" + fileName + "\">"
                            + newsName + "</a></li>");
                }
                index++;
            }
            ...
            //Telling user that the XML file is not valid
            out.println("The XML file IS NOT valid.");
        }
